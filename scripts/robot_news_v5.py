#!/usr/bin/env python3
"""
CasinoRadar - Robô de Geração Automática de Artigos v5
...
RSS_SOURCES = [
    # iGaming Business (global - mais confiável)
...
def fetch_rss(url: str) -> list[dict]:
    ...
    try:
        resp = requests.get(url, timeout=15, headers={"User-Agent": "CasinoRadar-Bot/1.0"}, verify=False)
...
def main():
    print(f"\n{'='*60}")
    print(f"CasinoRadar Robot v5 - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"{'='*60}\n")
